...
export default class Router {
  ...
  async route(context) {
    ...
    if (components.length === 1) {
      ...
      const middleware = endpointMiddleware // eslint-disable-next-line no-shadow
        .concat(context => evaluateMethod(context, methodMap, endpointNamePlural));
    ...
    if (components.length === 2) {
      ...
      const middleware = endpointMiddleware // eslint-disable-next-line no-shadow
        .concat(context => evaluateMethod(context, methodMap, endpointName, endpointNamePlural));
      const composedMiddleware = compose(middleware);
      return composedMiddleware(context);
    }
